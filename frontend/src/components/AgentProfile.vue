<template>
  <div class="d-flex flex-column align-center justify-center pa-3">
    <!-- Agent Info -->
    <div v-if="selectedAgent" class="text-center mb-4">
      <div class="agent-icon mb-2" :style="{ color: selectedAgent.color, fontSize: '3rem' }">
        {{ selectedAgent.icon }}
      </div>
      <h2 class="agent-name mb-1" :style="{ color: selectedAgent.color }">
        {{ selectedAgent.name }}
      </h2>
    ...
    <!-- Avatar (Lead Profile) -->
    <v-avatar class="mb-4" rounded="lg" @click="triggerAvatarUpload" style="cursor: pointer; width: 300px; height: 200px;">
    ...
    <div class="text-center mt-2">
      <p class="text-body-2 text-medium-emphasis">
        {{ conversationStarted ? 'Conversación en progreso...' : 'Listo para iniciar demo' }}
      </p>
    </div>
  </div>
</template>
